...
export class FileGenerator {
	...
	static async generate(config: PluginConfig): Promise<File[]> {
		...
		if (config.database.entities.length > 0) {
			const entityClasses = await this.loadModule(version, GeneratorLanguages.CSHARP, 'DatabaseEntityGenerator');
			const nameCompatibility = await this.loadModule(version, GeneratorLanguages.CSHARP, 'NameCompatibilityGenerator');
			const schemaMigration = await this.loadModule(version, GeneratorLanguages.CSHARP, 'SchemaMigrationGenerator');

			generators.push(entityClasses);
			generators.push(nameCompatibility);
			generators.push(schemaMigration);
		}
