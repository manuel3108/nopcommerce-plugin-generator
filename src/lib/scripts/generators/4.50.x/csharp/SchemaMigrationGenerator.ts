...
export default class SchemaMigrationGenerator450 implements IFileGenerator {
	...
	protected generateContent(config: PluginConfig, path: string[], className: string): string {
		...
		const myClass = new Class(generateClassNamespace(config.base.nameSpace, path), className, {
			addCtor: true,
			addRegions: true,
			attributes: [new FieldAttribute(`NopMigration("${date}", "${config.details.systemName} base schema", MigrationProcessType.NoMatter)`)]
		});

		myClass.usings.push(new Using(config.base.nameSpace + '.Domain'));
		myClass.usings.push(new Using('Nop.Data.Migrations'));
		myClass.usings.push(new Using('Nop.Data.Extensions'));
		...
		config.database.entities.forEach((entity) => {
			migrateUp.expressions.push(`Create.TableFor<${entity.className}>();`);
		});
